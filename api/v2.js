...
.replace(/<u>(.*?)<\/u>/g,' $1 ')
.replace(/<b>(.*?)<\/b>/g,' $1 ')
.replace(/<i>(.*?)<\/i>/g,' $1 ').replace(/<[^>]+>/g,'')
.replace(/ /g,' ').replace(/ &/g,' &').replace(/­/g,'')
.replace(/&#(\d+);/g,(_,c)=>String.fromCharCode(+c))
.replace(/\/[^\s,]+/g, '')
.replace(/[\u2070-\u2079\u00b9\u00b2\u00b3]+/g, '')
// ИСПРАВЛЕНИЕ: убираем скобки с буквой: sprech(e) -> spreche
.replace(/\(([a-zäöüß]?)\)/gi, '$1')
// ИСПРАВЛЕНИЕ: убираем пробелы между буквами: geg angen -> gegangen
.replace(/(\w)\s+(\w)/g, '$1$2')
.replace(/\s+/g, ' ').trim();
}
...
if (rs === -1) break;const re = tableHtml.indexOf('', rs);
if (re === -1) break;
...
const niveau = niveauM ? niveauM[1] : '';// Verb type
...
while (true) {
const td = rowHtml.indexOf('<td', cp);if (td === -1) break;
const tde = rowHtml.indexOf('', td);
if (tde === -1) break;
