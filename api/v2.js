...
function formatCell(h) {  return h
    .replace(/ (. ?) <\/u><\/b>/g,' $1 ')
    .replace(/ (. ?) <\/u><\/i><\/b>/g,' $1 ')
    .replace(/ (. ?) <\/u>/g,' $1 ')
    .replace(/ (. ?) <\/b>/g,' $1 ')
    ...
    .replace(//[^\s,]+/g, '')
    .replace(/[\u2070-\u2079\u00b9\u00b2\u00b3]+/g, '')
    // ИСПРАВЛЕНИЕ 1: Убираем скобки (e) -> e
    .replace(/\(([a-zäöüß]?)\)/gi, '$1')
    // ИСПРАВЛЕНИЕ 2: Убираем пробелы между буквами: "ge g angen" -> "gegangen"
    .replace(/(\w)\s+(\w)/g, '$1$2')
    .replace(/\s+/g, ' ').trim();
}
...
function parse(html, word) {
  ...
  const pronM = html.match(//[a-z\u0250-\u02ff\u00e6\u00f8\u0259\u026aː.]+//);  if (pronM) {
  ...
  // Hauptformen (ОБЪЯВЛЯЕМ ЗДЕСЬ, ЧТОБЫ ИСПОЛЬЗОВАТЬ В ЛОГИКЕ ТИПА)
  ...
  // === ОПРЕДЕЛЕНИЕ ТИПА ГЛАГОЛА (АЛГОРИТМИЧЕСКОЕ) ===  // Проверяем окончания Präteritum (-te) и Partizip II (-t)
  ...
  let verbType = 'unregelmäßig'; // По умолчанию неправильный
  if (pt3Clean && pf3Clean) {
    // Если Präteritum на -te И Partizip на -t -> Правильный
    if (pt3Clean.endsWith('te') && pf3Clean.endsWith('t')) {
      verbType = 'regelmäßig';
    }
  }
  // =================================================
  ...
  const mp3Segs = {
    ...
    perfekt:     'indikativ/perfekt/',    konjunktiv2: 'konjunktiv/praeteritum/',
  };
