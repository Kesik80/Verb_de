...
function formatCell(h) {
  return h
    ...
    .replace(/\/[^\s,]+/g, '')
    .replace(/[\u2070-\u2079\u00b9\u00b2\u00b3]+/g, '')
    // Убираем скобки: sprech(e) -> spreche
...
function parse(html, word) {
  ...
  // Hauptformen  const p3 = strip(tenses.praesens?.['er/sie/es'] || '');
  ...
  const mp3s = {};
  const mp3Segs = {
    praesens: 'indikativ/praesens/',    praeteritum: 'indikativ/praeteritum/',
