...
function parse(html, word) {
  ...
  // Verb type — primary check in header area
  let unregelmaessig = false;
  let regelmaessig = false;
  const headerChunk = html.slice(0, Math.min(html.length, niveauM ? niveauM.index + 500 : 3000));
  const rInfHeaderM = html.match(/class="[^"]*rInf[^"]*"[^>]*>([\s\S]{0,200}?)<\/p>/);
  const checkZone = (rInfHeaderM ? rInfHeaderM[1] : '') + headerChunk;
  unregelmaessig = /\u043d\u0435\u043f\u0440\u0430\u0432\u0438\u043b\u044c\u043d/i.test(checkZone);
  regelmaessig = /\u043f\u0440\u0430\u0432\u0438\u043b\u044c\u043d/i.test(checkZone) && !unregelmaessig;
